        ...
        <div class="container">
            <div class="row justify-content-between align-items-center d-flex">
                <div class="col-lg-8 top-left">
                    <h1 class="text-white mb-20">Article Details</h1>
                    <ul>
                        <li>
                            <a href="/">Home</a
                        ...
                        <div class="single-page-post">
                            ...
                            <div class="top-wrapper">
                                ...
                                <div class="row d-flex justify-content-between pl-3 mt-2">
                                    {{--                                    {{route('category.post', $article->category->slug)}}--}}
                                    <h4 class="col-lg-8 col-md-12 text-muted mt-3"><a
                                            href="">{{$article->category->name}}</a></h4>
                                    <div class="col-lg-4 col-md-12 right-side d-flex justify-content-end">
                                        <div class="desc">
                                            <h2>{{$article->user->name}}</h2>
                                            <h3>{{$article->created_at->diffForHumans()}}</h3>
                                        </div>
                                        <div class="user-img">
                                            <img src="{{asset('storage/'.$article->user->image)}}"
                                                 alt="{{$article->user->name}}" width="50px"/>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="tags">
                                <ul>
                                    @foreach ($article->tags as $tag)
                                        {{--                                        {{route('tag.posts', $tag->name)}}--}}
                                        <li><a href="/">#{{$tag->name}}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                            <div class="single-post-content"
                                 style="all: initial;font-family: sans-serif;line-height: 1.8;">
                                {!! $article->body !!}
                            </div>
                                    ...
                                    <div class="col-lg-4 single-b-wrap col-md-12">
                                        @guest
                                            <i class="fa fa-heart-o"
                                               aria-hidden="true"></i>{{$article->likedUsers->count()}} people like this
                                        @else
                                            <a href="#"
                                               onclick="document.getElementById('like-form-{{$article->id}}').submit();">
                                                <i class="fa fa-heart" aria-hidden="true"
                                                   style="color: {{Auth::user()->likedArticles()->where('article_id', $article->id)->count() > 0 ? 'red' : ''}}"></i></a>
                                            {{$article->likedUsers->count()}} people like this
{{--                                        {{route('post.like',$article->id)}}--}}
                                            <form action="/" method="POST"
                                                  style="display: none" id="like-form-{{$article->id}}">
                                                @csrf
                                            </form>
                                        @endguest
                                    </div>
                                    <div class="col-lg-4 single-b-wrap col-md-12">
                                        <i class="fa fa-eye" aria-hidden="true"></i> {{$article->views->count()}} views
                                    </div>
                                    <div class="col-lg-4 single-b-wrap col-md-12">
                                        <i class="fa fa-comment-o"
                                           aria-hidden="true"></i> {{$article->comments->count()}} comments
                                    </div>
                                    <div class="row mx-1">
                                        ...
                                        <div class="col-lg-6 single-b-wrap col-md-12 mt-3">
                                            <button class="btn btn-primary" id="shareBtn" style="display: none"><i
                                                    class="fa fa fa-share text-white" aria-hidden="true"></i> Share
                                            </button>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <section class="commentform-area">
                                @guest
                                    <div class="container">
                                        <h4>Please Sign in to post comments - <a href="{{route('login')}}">Sing in</a>
...
{{--                                                {{route('comment.store', $article->id)}}--}}
                                                <form action="/" method="POST">
                                                    ...
                                                    <textarea
                                                        ...
                                                        required=""
                                                    ></textarea>
                                                    <button type="submit" class="primary-btn mt-20" href="#">Comment
                                                    </button>
                                                </form>
                                            </div>
                                        </div>
                            ...
                            <section class="comment-sec-area">
                                <div class="container">
                                    <div class="row flex-column">
                                        <h5 class="text-uppercase pb-80">{{$article->comments->count()}} Comments</h5>
                                        <br/>
                                                ...
                                                <div class="comment-list">
                                                    <div class="single-comment justify-content-between d-flex">
                                                        <div class="user justify-content-between d-flex">
                                                            <div class="thumb">
                                                                <img
                                                                    src="{{asset('storage/user/'.$comment->user->image)}}"
                                                        ...
                                                        <div class="comment-list left-padding">
                                                            ...
                                                            >
                                                                <div class="user justify-content-between d-flex">
                                                                    <div class="thumb">
                                                                        <img
                                                                            src="{{asset('storage/user/'.$reply->user->image)}}"
...
@push('footer')
    <script type="text/javascript">
        ...
        let postTitle = encodeURI('{{$article->title}}');
